...
export class Namespace extends EventEmitter {
   ...
   */
  public use(fn: Function): Namespace {
   ...
   */
  private run(socket: Socket, fn: Function): void {
   ...
   * @return {Socket}
   */
  private add(client: Client, query, fn): Socket {
   ...
   * @return {Namespace} self
   */
  public clients(fn: Function): Namespace {
    if (!this.adapter) {
      throw new Error(
